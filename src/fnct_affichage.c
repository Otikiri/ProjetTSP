...
void affichage_help(){
    ...
    printf("> %s./bin/tsp -f test/FICHIER -m [bf,nn,rw,2optnn,2optrw] %s: pour calculer la distance d'une tournee avec des algorithmes differents\n",GREEN,NORMAL);
    printf("> %s./bin/tsp -f test/FICHIER -m [ga,gadpx] <nombre d'individu> <nombre de generation> <taux de mutation>  %s: pour calculer la distance d'une tournee avec des algorithmes differents\n",GREEN,NORMAL);
